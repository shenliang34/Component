package
{
	...
	public class SortonUtil
	{
		...
		{ 
			...
			{ 
				if(isChinese(chinese.charAt(i))){
					ret += convertChar(chinese.charAt(i),i); 
				}else{
					ret += chinese.charAt(i);
				}
			} 
			return ret; 
		} 
		 ...
		 * 
		 */   
		private static function convertChar(chineseChar:String,i:int):String 
		{ 
		...
		{ 
			...
			{
				if (convertChar(chineseChar,i) == "\0") 
				{ 
					return false; 
				} 
			}
		 ...
		 * 
		 */		
		public static function sortByChinese(arr:Array,sortKey:Array = null,sortType:Array = null,chineseKey:String = ""):Array
		{
			...
			{ 
				if(chineseKey == ""){
					...
					var chineseName:String = convertString(item[chineseKey]);
					var o:Object = {object: item,a: chineseName};
					for each (var j:String in sortKey) 
					{
						o[j] = item[j]; 
					}
					sortedArr[sortedArr.length] = o;
//					byte.writeMultiByte(chineseName, "gb2312"); 
				}
			} 
//			byte.position = 0; 
...
//				
//			} 
			sortKey = sortKey.concat(["a"]);
			sortType = sortType.concat([Array.CASEINSENSITIVE]);
			sortedArr.sortOn(sortKey, sortType); 
